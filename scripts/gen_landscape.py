...
RADIUS = 2
# A road may change at most this many levels per hex (the validator's rule);
# where the terrain would make it climb faster, the ground along the road is
# cut down: fully within ROAD_GRADE_FLAT_MM of the centreline, fading over
# ROAD_GRADE_FADE_MM beyond it.
MAX_ROAD_STEP_LEVELS = 1
ROAD_GRADE_FLAT_MM = 15.0
ROAD_GRADE_FADE_MM = 25.0
...
def main(argv: list[str] | None = None) -> None:
    ...
    def clip_level(value: float) -> int:
        return int(min(max(round(value), 0), preset.level_count - 1))

    # Road grading. Stations along the road's world polyline (crossing,
    # ring hex, centre hex, ring hex, crossing per flower) get the terrain's
    # rounded level; a min-plus limiter forward and back cuts every station
    # that would sit more than MAX_ROAD_STEP_LEVELS above its neighbour, and
    # the (never positive) difference is blended into the field around the
    # road, so the road climbs one level per hex at most and the ground next
    # to it is graded with it.
    road_poly: list[tuple[float, float]] = []
    for cell in preset.road:
        ox, oy = origin[cell]
        entry, exit_side = road[cell]
        pts = [
            layout.junction_center(entry),
            *(layout.cell_center(h) for h in FlowerLayout.path_route(entry, exit_side)),
            layout.junction_center(exit_side),
        ]
        for x, y in pts:
            world = (ox + x, oy + y)
            if not road_poly or math.hypot(world[0] - road_poly[-1][0], world[1] - road_poly[-1][1]) > 1e-6:
                road_poly.append(world)
    road_s = [0.0]
    for (ax, ay), (bx, by) in zip(road_poly, road_poly[1:]):
        road_s.append(road_s[-1] + math.hypot(bx - ax, by - ay))
    base_levels = [clip_level(elevation(x, y)) for x, y in road_poly]
    graded = list(base_levels)
    for i in range(1, len(graded)):
        graded[i] = min(graded[i], graded[i - 1] + MAX_ROAD_STEP_LEVELS)
    for i in range(len(graded) - 2, -1, -1):
        graded[i] = min(graded[i], graded[i + 1] + MAX_ROAD_STEP_LEVELS)
    road_offset = [g - b for g, b in zip(graded, base_levels)]

    def road_grade(x: float, y: float) -> float:
        best_d, best_s = float("inf"), 0.0
        for i, ((ax, ay), (bx, by)) in enumerate(zip(road_poly, road_poly[1:])):
            dx, dy = bx - ax, by - ay
            t = ((x - ax) * dx + (y - ay) * dy) / (dx * dx + dy * dy)
            t = min(max(t, 0.0), 1.0)
            d = math.hypot(x - (ax + t * dx), y - (ay + t * dy))
            if d < best_d:
                best_d, best_s = d, road_s[i] + t * math.hypot(dx, dy)
        i = max(0, min(len(road_s) - 2, next((k for k in range(len(road_s) - 1) if road_s[k + 1] >= best_s), len(road_s) - 2)))
        span = road_s[i + 1] - road_s[i]
        u = (best_s - road_s[i]) / span if span > 0 else 0.0
        offset = road_offset[i] + (road_offset[i + 1] - road_offset[i]) * u
        w = math.exp(-((max(best_d - ROAD_GRADE_FLAT_MM, 0.0) / ROAD_GRADE_FADE_MM) ** 2))
        return w * offset

    def level_at(x: float, y: float) -> int:
        return clip_level(elevation(x, y) + road_grade(x, y))
    ...
    for index, cell in enumerate(cells):
        ...
        if cell in river:
            ...
            lines.append("    water: []")
        lines.append("")
    # the validator's road rule, checked here so a bad preset fails loudly
    for cell in preset.road:
        ox, oy = origin[cell]
        entry, exit_side = road[cell]
        stations = []
        for side in (entry,):
            c = layout.side_corners(side)
            stations.append(0.5 * (level_at(ox + c[1][0], oy + c[1][1]) + level_at(ox + c[2][0], oy + c[2][1])))
        stations += [level_at(ox + layout.cell_center(h)[0], oy + layout.cell_center(h)[1]) for h in FlowerLayout.path_route(entry, exit_side)]
        c = layout.side_corners(exit_side)
        stations.append(0.5 * (level_at(ox + c[1][0], oy + c[1][1]) + level_at(ox + c[2][0], oy + c[2][1])))
        for a, b in zip(stations, stations[1:]):
            if abs(b - a) > MAX_ROAD_STEP_LEVELS + 1e-9:
                raise SystemExit(f"{flower_id(preset.prefix, *cell)}: road stations {stations} step more than {MAX_ROAD_STEP_LEVELS} level")
    out.write_text("\n".join(lines))
    counts = {}
    for cell in cells:
        ox, oy = origin[cell]
